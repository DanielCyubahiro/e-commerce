...
Forking this repo onto a different database or ORM means writing a new pair of
adapters and rewiring one module. The procedure:
...
1. Implement `ProductReadRepository` and `ProductWriteRepository` from
   ...
   not just its signature: `ProductWriteRepository.add` throws
   `DuplicateSkuException` on a colliding SKU rather than pre-checking (so two
   concurrent callers cannot both pass a check and then collide), and `delete`
   returns `false` rather than throwing when no row matched that id.
2. Write one binding file per contract suite. For the write side, follow
...
3. Register the two adapters against `PRODUCT_READ_REPOSITORY` and
   ...
   `useClass: DrizzleProductWriteRepository` providers. Nothing else in the
   module, and no command or query handler, needs to change.
