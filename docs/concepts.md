...
| ordering | [`PlaceOrderCommand`](../src/ordering/application/use-cases/commands/place-order/place-order.command.ts), [`CancelOrderCommand`](../src/ordering/application/use-cases/commands/cancel-order/cancel-order.command.ts) | `PlaceOrderCommand` carries the idempotency key as `string \| null`, beside the order's fields rather than among them; `CancelOrderCommand` carries an `OrderScope`, so one command serves the owner and staff |
...
| ordering | [`PlaceOrderHandler`](../src/ordering/application/use-cases/commands/place-order/place-order.handler.ts), [`CancelOrderHandler`](../src/ordering/application/use-cases/commands/cancel-order/cancel-order.handler.ts) | the first handlers here to run two ports inside one `UnitOfWork`; `PlaceOrderHandler` is the only place a stock shortfall becomes `StockUnavailableException`, and it throws a private sentinel inside the transaction to unwind an allocation when the insert loses the idempotency race |
